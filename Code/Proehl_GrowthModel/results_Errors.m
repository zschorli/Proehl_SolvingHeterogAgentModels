...
legend([p1,p2,p3,p4,p5],'Krusell-Smith', 'Reiter', 'Proximal Point Algo', 'boom', 'recession','Location','best','AutoUpdate','off')
legend('boxoff')
...
dyn_pdf_KS = pdf_KS;
dyn_cdf_KS = cdf_KS;
K_KS = zeros(Res.nsamples,1);
sd_KS = zeros(Res.nsamples,1);
skew_KS = zeros(Res.nsamples,1);
kurt_KS = zeros(Res.nsamples,1);
gini_KS = zeros(Res.nsamples,1);
dyn_euler_K_KS = zeros(Res.nsamples,1);
K_pr_LoM_KS = zeros(Res.nsamples,1);
idx_posProb_KS = zeros(2,length(Res.kGrid));
idx = max(find(pdf_KS(1,:)>eps,1,'last'),find(pdf_KS(2,:)>eps,1,'last'));
...
dyn_cdf = cdf;
dyn_weights = weights;
kb = zeros(2,M+1,Res.nsamples,1);
kg = zeros(2,M+1,Res.nsamples,1);
cb = zeros(2,M+1,Res.nsamples,1);
cg = zeros(2,M+1,Res.nsamples,1);
...
for i=1:Res.nsamples
    ...
    for order_ct=0:M
        ...
        kb(method+1,order_ct+1,i) = interp1(StaticParams.kGrid_pol,squeeze(...
                  getCApprox(Sol.distrGrid,squeeze(Sol.k_prime(:,2*Res.z_ag(i)+1,:)),Sol.idx,squeeze(weights(method+1,order_ct+1,1:(Poly.phi_N+1)))'))',kb_0)';
        kg(method+1,order_ct+1,i) = interp1(StaticParams.kGrid_pol,squeeze(...
                  getCApprox(Sol.distrGrid,squeeze(Sol.k_prime(:,2*Res.z_ag(i)+2,:)),Sol.idx,squeeze(weights(method+1,order_ct+1,1:(Poly.phi_N+1)))'))',kg_0)';
        cb(method+1,order_ct+1,i) = interp1(StaticParams.kGrid_pol,squeeze(...
                  getCApprox(Sol.distrGrid,squeeze(Sol.c(:,2*Res.z_ag(i)+1,:)),Sol.idx,squeeze(weights(method+1,order_ct+1,1:(Poly.phi_N+1)))'))',kb_0)';   
        cg(method+1,order_ct+1,i) = interp1(StaticParams.kGrid_pol,squeeze(...
                  getCApprox(Sol.distrGrid,squeeze(Sol.c(:,2*Res.z_ag(i)+2,:)),Sol.idx,squeeze(weights(method+1,order_ct+1,1:(Poly.phi_N+1)))'))',kg_0)';   
    ...
    K_KS(i) = StaticParams.kGrid*squeeze(pdf_KS)'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)];
    sd_KS(i) = sqrt(StaticParams.kGrid.^2*pdf_KS'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)]);
    skew_KS(i) = ((StaticParams.kGrid-squeeze(K_KS(i))).^3 ...
                 *pdf_KS'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)])...
                 ./squeeze(sd_KS(i))^3;
    kurt_KS(i) = ((StaticParams.kGrid-squeeze(K_KS(i))).^4 ...
                 *pdf_KS'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)])...
                 ./squeeze(sd_KS(i))^4;
    gini_KS(i) = sum(sum((repmat(StaticParams.kGrid',[1,length(StaticParams.kGrid)])-repmat(StaticParams.kGrid,[length(StaticParams.kGrid),1]))...
                                  .*repmat((pdf_KS'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)]),[1,length(StaticParams.kGrid)])...
                                  .*repmat((pdf_KS'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)])',[length(StaticParams.kGrid),1]),2),1)...
                                  ./(2*squeeze(K_KS(i)));
    k_prime_KS = [interpn(KS.k,KS.km,KS.kprime(:,:,Res.z_ag(i)+1,1),Res.kGrid,min(KS.km_max,max(KS.km_min,K_KS(i))))';...
                  interpn(KS.k,KS.km,KS.kprime(:,:,Res.z_ag(i)+1,2),Res.kGrid,min(KS.km_max,max(KS.km_min,K_KS(i))))'];       
    c_prime_KS = max(0,wealth(Res.z_ag(i),K_KS(i),Res.kGrid)-k_prime_KS);
    % standard Euler policies 
    euler_K = [StaticParams.ur(Res.z_ag(i)+1),StaticParams.er(Res.z_ag(i)+1)]...
              *sum(interp1(Res.kGrid,sort(k_prime_KS,2)',StaticParams.kGrid)'.*pdf_KS,2);
    K_pr_LoM_KS(i) = exp(KS.B1(2*Res.z_ag(i)+1)+KS.B1(2*Res.z_ag(i)+2)*log(K_KS(i)));
    euler_expec = @(grid) repmat([1-StaticParams.delta+rate(0,euler_K);...
                                  1-StaticParams.delta+rate(1,euler_K)],[1,length(grid)])...
                  .*max(1e-15,([wealth(0,euler_K,grid);wealth(1,euler_K,grid)]...
                        -interp1(StaticParams.kGrid,...
                         ...
                         grid,'linear','extrap')'))...
                  .^(-StaticParams.gamma);
    euler_c_prime_KS = min(wealth(Res.z_ag(i),K_KS(i),Res.kGrid),...
    ...
    Err = Err(logical(idx_posProb_KS));
    Err_algoGrid = Err_algoGrid(logical(squeeze(idx_posProb_KS(:,algo_idx))));
    Res.stErr_KS1 = [Res.stErr_KS1;Err];
    Res.stErr_KS1_algoGrid = [Res.stErr_KS1_algoGrid;Err_algoGrid];
    % update distribution
    if i<Res.nsamples
        pdf_KS = calcEoPDistr(interp1(Res.kGrid,sort(k_prime_KS,2)',StaticParams.kGrid)',...
                 cdf_KS,StaticParams.kGrid);        
        pdf_KS = (repmat(1./(StaticParams.p(2*Res.z_ag(i)+(1:2))'*StaticParams.P(2*Res.z_ag(i)+(1:2),2*Res.z_ag(i+1)+(1:2)))',[1,2])...
                   .*StaticParams.P(2*Res.z_ag(i)+(1:2),2*Res.z_ag(i+1)+(1:2))')...
                   *(repmat(StaticParams.p(2*Res.z_ag(i)+(1:2)),[1,size(pdf_KS,2)])...
                   .*pdf_KS);
        idx = max(find(squeeze(pdf_KS(1,:))>eps,1,'last'),find(squeeze(pdf_KS(2,:))>eps,1,'last'));
        idx_posProb_KS(:,(Res.kGrid<=StaticParams.kGrid(idx))) = 1;
        pdf_KS = pdf_KS.*(pdf_KS>eps);
        pdf_KS = pdf_KS./repmat(sum(pdf_KS,2),[1,size(pdf_KS,2)]);   
        cdf_KS = cummax(min(1,cumsum(pdf_KS,2)),2);
        cdf_KS(1,cdf_KS(1,:)==max(cdf_KS(1,:))) = 1;
        cdf_KS(2,cdf_KS(2,:)==max(cdf_KS(2,:))) = 1; 
    end
    
    % dynamic Euler policies and distributions
    dyn_euler_K_KS(i) = StaticParams.kGrid*dyn_pdf_KS'*[StaticParams.ur(Res.z_ag(i)+1);StaticParams.er(Res.z_ag(i)+1)];
    k_prime_next = [interpn(KS.k,KS.km,KS.kprime(:,:,Res.z_ag(i)+1,1),Res.kGrid,min(KS.km_max,max(KS.km_min,dyn_euler_K_KS(i))))';...
                    interpn(KS.k,KS.km,KS.kprime(:,:,Res.z_ag(i)+1,2),Res.kGrid,min(KS.km_max,max(KS.km_min,dyn_euler_K_KS(i))))'];       
    euler_K = [StaticParams.ur(Res.z_ag(i)+1),StaticParams.er(Res.z_ag(i)+1)]...
              *sum(interp1(Res.kGrid,sort(k_prime_next,2)',StaticParams.kGrid)'.*dyn_pdf_KS,2);
    euler_expec = @(grid) repmat([1-StaticParams.delta+rate(0,euler_K);...
                                  1-StaticParams.delta+rate(1,euler_K)],[1,length(grid)])...
                  .*max(1e-15,([wealth(0,euler_K,grid);wealth(1,euler_K,grid)]...
                        -interp1(StaticParams.kGrid,...
                         ...
                         grid','linear','extrap')'))...
                  .^(-StaticParams.gamma);
    dyn_euler_c_prime_KS = min(wealth(Res.z_ag(i),dyn_euler_K_KS(i),Res.kGrid),...
    ...
    dyn_euler_k_prime_KS = wealth(Res.z_ag(i),dyn_euler_K_KS(i),Res.kGrid)-dyn_euler_c_prime_KS;
    % dynamic error
    ...
    Err = Err(logical(dyn_idx_posProb_KS));
    Err_algoGrid = Err_algoGrid(logical(squeeze(dyn_idx_posProb_KS(:,algo_idx))));
    Res.dynErr_KS1 = [Res.dynErr_KS1;Err];
    Res.dynErr_KS1_algoGrid = [Res.dynErr_KS1_algoGrid;Err_algoGrid];
    % update distribution
    if i<Res.nsamples
        dyn_pdf_KS = calcEoPDistr(interp1(Res.kGrid,sort(dyn_euler_k_prime_KS,2)',StaticParams.kGrid)',...
                     dyn_cdf_KS,StaticParams.kGrid);
        dyn_pdf_KS = (repmat(1./(StaticParams.p(2*Res.z_ag(i)+(1:2))'*StaticParams.P(2*Res.z_ag(i)+(1:2),2*Res.z_ag(i+1)+(1:2)))',[1,2])...
                   .*StaticParams.P(2*Res.z_ag(i)+(1:2),2*Res.z_ag(i+1)+(1:2))')...
                   *(repmat(StaticParams.p(2*Res.z_ag(i)+(1:2)),[1,size(dyn_pdf_KS,2)])...
                   .*dyn_pdf_KS);  
        idx = max(find(squeeze(dyn_pdf_KS(1,:))>eps,1,'last'),find(squeeze(dyn_pdf_KS(2,:))>eps,1,'last'));
        dyn_idx_posProb_KS(:,(Res.kGrid<=StaticParams.kGrid(idx))) = 1;
        dyn_pdf_KS = dyn_pdf_KS.*(dyn_pdf_KS>eps);
        dyn_pdf_KS = dyn_pdf_KS./repmat(sum(dyn_pdf_KS,2),[1,size(dyn_pdf_KS,2)]);
        dyn_cdf_KS = cummax(min(1,cumsum(dyn_pdf_KS,2)),2);
        dyn_cdf_KS(1,dyn_cdf_KS(1,:)==max(dyn_cdf_KS(1,:))) = 1;
        dyn_cdf_KS(2,dyn_cdf_KS(2,:)==max(dyn_cdf_KS(2,:))) = 1;  
    end
    ...
    if i<Res.nsamples
        ...
        set(gca,'nextplot','add');
        p1 = plot(StaticParams.kGrid,2*StaticParams.p(2*Res.z_ag(i+1)+(1:2))'*pdf_KS,'m','LineWidth',2);
        set(gca,'nextplot','add');
        p2 = plot(StaticParams.kGrid,2*StaticParams.p(2*Res.z_ag(i+1)+(1:2))'*pdf_R,'c','LineWidth',2);
        set(gca,'nextplot','add');
        p3 = plot(StaticParams.kGrid,2*StaticParams.p(2*Res.z_ag(i+1)+(1:2))'*squeeze(pdf(1,M,:,:)),'b','LineWidth',2);
        set(gca,'nextplot','replacechildren');
        frame = getframe(gcf);
        writeVideo(v,frame);
    end
...
Res.LoMErr_KS1 = (Res.K_pr_LoM_KS(1:end-1)-Res.K_KS(2:end))./Res.K_KS(2:end);
