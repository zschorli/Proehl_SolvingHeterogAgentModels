...
switch Params.case_nr
  ...
  Params.delta = 0.025; % deprecitation, quarterly  0.15;%
...
%   end
  ...
  Params.KAggrGrid =Sol.agK; %linspace(Params.lowB(1),Params.uppB(1),Params.nK)';
  Params.lowB = min(Params.KAggrGrid);
  Params.uppB = max(Params.KAggrGrid);
  %   take logs of xEqu?
...
%   Params.KIndGrid = logspaceshift(kmin,kmax*1.001,Params.n,kshift)';
  ...
  bound = 2; 
  Params.KIndGrid = unique([linspace(kmin,bound,36),...
                    bound+(kmax-bound)*ygrid])';
  clear xgrid ygrid bound Sol;
