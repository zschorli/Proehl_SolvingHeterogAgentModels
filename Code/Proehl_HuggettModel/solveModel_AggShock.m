...
switch case_nr
    case {1,7}
        Sol.distrGrid_min = [0, 0.2, -0.01, -5e-3, -0.01];
        Sol.distrGrid_max = [0, 0.57, 0.04,  0.05,  0.02];
    case 3
        Sol.distrGrid_min = [0, 0.2,  0,    -0.02, -0.02];
        Sol.distrGrid_max = [0, 0.57, 0.06,  0.15,  0.05];
    case 4
        Sol.distrGrid_min = [0, 0.2, -0.01, -5e-3, -0.01];
        Sol.distrGrid_max = [0, 0.57, 0.02,  0.03,  0.01];
    case 5
        Sol.distrGrid_min = [0, 0.2, -0.01, 4e-3, -1e-3];
        Sol.distrGrid_max = [0, 0.55, 0.15, 0.015, 2e-3];
    case 6
        Sol.distrGrid_min = [0, 0.2, -0.01, -5e-3,  -0.01];
        Sol.distrGrid_max = [0, 0.5,  0.02,  0.025,  0.01];
    case 8
        Sol.distrGrid_min = [0, 0.2, -0.02, -0.01, -0.01];
        Sol.distrGrid_max = [0, 0.6,  0.03,  0.04,  0.02];
    case 9
        Sol.distrGrid_min = [0, 0.15, -0.025, -3e-3, -2e-3];
        Sol.distrGrid_max = [0, 0.45,  0.01,   0.03,  1e-3];
    case {2,10,11,12,13}
        Sol.distrGrid_min = [0, 0.01, 0,    -0.08, -0.1];
        Sol.distrGrid_max = [0, 0.45, 0.16,  0.3,   0.4];
end
...
for m=1:grid_N
    ...
    Sol.p(m,:) = [preSol.price_b,preSol.price_g];%1.6*ones(1,2);% 
...
if StaticParams.methodEqPFI == 1
    ...
    = OptPol_AggShock_PFI(Sol.p,Sol.c,Sol.k_prime,grid,StaticParams,Poly,Sol.distrGrid,Sol.idx,Sol.wealth,Sol.wage);
    Sol.time_solve = toc;
else
