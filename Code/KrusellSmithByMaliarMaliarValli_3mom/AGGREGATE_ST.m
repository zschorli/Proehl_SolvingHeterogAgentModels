...
function [kmts,kvarts,kskewts,kcross_out]  = AGGREGATE_ST(T,idshock,agshock,km_max,km_min,kvar_max,kvar_min,kskew_max,kskew_min,kprime,km,kvar,kskew,k,epsilon2,k_min,k_max,kcross,a2);

kmts=zeros(T,1);         % a time series of the mean of capital distribution 
kvarts=zeros(T,1);
kskewts=zeros(T,1);
kmts_b=zeros(T,1);         % a time series of the mean of capital distribution 
kvarts_b=zeros(T,1);
kskewts_b=zeros(T,1);
kcross_out = kcross;
...
for t=1:T
   ...
   kprimet4=interpn(k,km,kvar,kskew,a2,epsilon2,kprime,k, kmts_b(t),kvarts_b(t),kskewts_b(t),agshock(t),epsilon2,'spline');
   ...
   kcross=kcrossn;
   if kmts(t)==kmts_b(t) && kvarts(t)==kvarts_b(t) && kskewts(t)==kskewts_b(t)
      kcross_out = kcrossn;
   end
