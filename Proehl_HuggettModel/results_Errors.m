...
legend([p1,p2,p4,p5],'low income', 'high income', 'boom', 'recession','Location','best')
legend('boxoff')
...
p_vector = linspace(0.7,1.4,13);
euler_c_prime_aux = zeros(2,length(Res.kGrid),length(p_vector));
...
for i=1:Res.nsamples
    ...
    for order_ct=1:M
        ...
        euler_expec = @(grid) (interp1(StaticParams.kGrid_pol,...
                               ...
                               grid,'linear','extrap')')...
                      .^(-StaticParams.gamma);
        euler_c_prime_aux = @(p) [(StaticParams.beta/p*StaticParams.P(2*Res.z_ag(i)+1,:)*euler_expec(k_prime(1,:))).^(-1/StaticParams.gamma);...
                                  (StaticParams.beta/p*StaticParams.P(2*Res.z_ag(i)+2,:)*euler_expec(k_prime(2,:))).^(-1/StaticParams.gamma)];
        ...
        k_prime_aux = @(p) max(StaticParams.k_min,(wealth(Res.z_ag(i),Res.kGrid)-euler_c_prime_aux(p))./p);
        ...
        euler_p = fzero(@(p) AggrAssetAlloc(p,k_prime_aux2(p),Grid_new,StaticParams,p,Res.z_ag(i),false,Poly),squeeze(pr(method+1,order_ct,i)),options);
        ...
        euler_expec = @(grid) (interp1(StaticParams.kGrid_pol,...
                               ...
                               grid,'linear','extrap')')...
                      .^(-StaticParams.gamma);
        euler_c_prime_aux = @(p) [(StaticParams.beta/p*StaticParams.P(2*Res.z_ag(i)+1,:)*euler_expec(interp1(StaticParams.kGrid_pol,sort(k_prime_next(1,:),2)',Res.kGrid)')).^(-1/StaticParams.gamma);...
                                  (StaticParams.beta/p*StaticParams.P(2*Res.z_ag(i)+2,:)*euler_expec(interp1(StaticParams.kGrid_pol,sort(k_prime_next(2,:),2)',Res.kGrid)')).^(-1/StaticParams.gamma)];
        Grid_new(1,:,:,:) = projectDistr(StaticParams.kGrid,squeeze(cdf(method+1,order_ct,1,:))',cdf_aux(1,:,:));
        Grid_new(2,:,:,:) = projectDistr(StaticParams.kGrid,squeeze(cdf(method+1,order_ct,Res.z_ag(i)+1,:))',cdf_aux(2,:,:));
        Grid_new(3,:,:,:) = projectDistr(StaticParams.kGrid,squeeze(cdf(method+1,order_ct,2,:))',cdf_aux(3,:,:));
        dyn_euler_k_prime = @(p) max(StaticParams.k_min,(wealth(Res.z_ag(i),Res.kGrid)-euler_c_prime_aux(p))./p);
        ...
        dyn_euler_p = fzero(@(p) AggrAssetAlloc(p,k_prime_aux2(p),Grid_new,StaticParams,p,Res.z_ag(i),false,Poly),squeeze(pr(method+1,order_ct,i)),options);
