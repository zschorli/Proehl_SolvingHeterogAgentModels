...
function [kmts,kvarts,kskewts,kkurtts,kcross]  = AGGREGATE_ST(T,idshock,agshock,km_max,km_min,kvar_max,kvar_min,kskew_max,kskew_min,kkurt_max,kkurt_min,kprime,km,kvar,kskew,kkurt,k,epsilon2,k_min,k_max,kcross,a2);
...
for t=1:T
   ...
   kvarts(t)=std(kcross);
   kvarts(t)=kvarts(t)*(kvarts(t)>=kvar_min)*(kvarts(t)<=kvar_max)+kvar_min*(kvarts(t)<kvar_min)+kvar_max*(kvarts(t)>kvar_max);
   kskewts(t)=(skewness(kcross));
   kskewts(t)=kskewts(t)*(kskewts(t)>=kskew_min)*(kskewts(t)<=kskew_max)+kskew_min*(kskewts(t)<kskew_min)+kskew_max*(kskewts(t)>kskew_max);
   kkurtts(t)=(kurtosis(kcross));
   kkurtts(t)=kkurtts(t)*(kkurtts(t)>=kkurt_min)*(kkurtts(t)<=kkurt_max)+kkurt_min*(kkurtts(t)<kkurt_min)+kkurt_max*(kkurtts(t)>kkurt_max);
   ...
   kprimet4=interpn(k,km,kvar,kskew,kkurt,a2,epsilon2,kprime,k, kmts(t),kvarts(t),kskewts(t),kkurtts(t),agshock(t),epsilon2,'spline');
