...
kaux(:,:,1,1)=repmat(k,[1,ngridkm]);
kaux(:,:,1,2)=repmat(k,[1,ngridkm]);
kaux(:,:,2,1)=repmat(k,[1,ngridkm]);
kaux(:,:,2,2)=repmat(k,[1,ngridkm]);

kmaux=zeros(ngridk,ngridkm,nstates_ag,nstates_id); % for the mean of capital 
                                                   % distribution (km)
kmaux(:,:,1,1)=repmat(km',[ngridk,1]);
kmaux(:,:,1,2)=repmat(km',[ngridk,1]);
kmaux(:,:,2,1)=repmat(km',[ngridk,1]);
kmaux(:,:,2,2)=repmat(km',[ngridk,1]);
