...
kprime=zeros(ngridk,ngridkm,ngridkvar,ngridkskew,nstates_ag,nstates_id); % next-period individual 
...
[kprime,c]  = INDIVIDUAL(prob,ur_b,ur_g,ngridk,ngridkm,ngridkvar,ngridkskew,nstates_ag,nstates_id,k,km,kvar,kskew,er_b,er_g,a,epsilon,l_bar,alpha,delta,gamma,beta,mu,km_max,km_min,kvar_max,kvar_min,kskew_max,kskew_min,kprime,B,B2,B3,criter_k,k_min,k_max,update_k);
            % compututing a solution to the individual problem

if Method==1; 
    [kmts,kvarts,kskewts,kcross1]  = AGGREGATE_ST(T,idshock,agshock,km_max,km_min,kvar_max,kvar_min,kskew_max,kskew_min,kprime,km,kvar,kskew,k,epsilon2,k_min,k_max,kcross,a2);
else % i.e., Method 2 
...
[B1(1:4),s2,s3,s4,s5]=regress(ybad,[ones(ibad,1) xbad x2bad x3bad]);R2bad=s5(1); 
    % run the OLS regression ln(km')=B(1)+B(2)*ln(km) for a bad agg. state 
    % and compute R^2 (which is the first statistic in s5)
[B1(5:8),s2,s3,s4,s5]=regress(ygood,[ones(igood,1) xgood x2good x3good]);R2good=s5(1);
    % make the OLS regression ln(km')=B(3)+B(4)*ln(km) for a good agg. state 
    % and compute R^2 (which is the first statistic in s5)
    
[B21(1:4),s2,s3,s4,s5]=regress(y2bad,[ones(ibad,1) xbad x2bad x3bad]);R2bad2=s5(1);
[B21(5:8),s2,s3,s4,s5]=regress(y2good,[ones(igood,1) xgood x2good x3good]);R2good2=s5(1);
[B31(1:4),s2,s3,s4,s5]=regress(y3bad,[ones(ibad,1) xbad x2bad x3bad]);R2bad3=s5(1);
[B31(5:8),s2,s3,s4,s5]=regress(y3good,[ones(igood,1) xgood x2good x3good]);R2good3=s5(1);
...
if dif_B>(criter_B*100) || dif_B2>(criter_B*100) || dif_B3>(criter_B*100)
...
%__________________________________________________________________________
%
save(strcat('KS3_Sol',num2str(case_nr),'.mat'));%Solution_to_model;
end